...
import { Menu, X, ChevronDown, Phone, Linkedin, Instagram } from 'lucide-react';
...
const Navbar = () => {
  ...
  // Client-side hydration check - Enhanced for static generation
  useEffect(() => {
    setIsClient(true);
  ...
  // Wait for navigation to be ready before positioning underline - Enhanced for SSG
  useEffect(() => {
    ...
    const checkNavReady = () => {
      ...
      if (activeIndex >= 0 && navLinksRef.current[activeIndex]) {
        ...
        if (activeLink && activeLink.offsetWidth > 0 && activeLink.offsetParent) {
          setNavReady(true);
          return true;
        }
      }
    ...
    // More aggressive checking for static sites
    const timeouts = [0, 50, 100, 150, 200, 300, 500, 750, 1000];
    const intervalChecks = [];
    
    timeouts.forEach(delay => {
      const timeout = setTimeout(() => {
        if (!navReady && checkNavReady()) {
          updateUnderlinePosition(false);
        }
      }, delay);
      intervalChecks.push(timeout);
    });

    // Also add an interval check for the first 2 seconds
  ...
  }, [isClient, isHydrated, pathname]);

  // Update underline when pathname changes - Enhanced
  useEffect(() => {
    if (navReady && isHydrated) {
  ...
  }, [pathname, navReady, isHydrated]);

  // Handle window resize
  useEffect(() => {
    if (!isClient || !navReady || !isHydrated) return;
  ...
  // Function to update the underline position - Enhanced error handling
  const updateUnderlinePosition = (animate = true) => {
    ...
    if (activeIndex >= 0 && navLinksRef.current[activeIndex]) {
      ...
      // More thorough checks for element readiness
      if (activeLink && activeLink.offsetParent && activeLink.offsetWidth > 0) {
  ...
  // Handle link click with animation - Enhanced for SSG
  const handleLinkClick = (e, link) => {
    ...
    if (linkIndex >= 0 && navLinksRef.current[linkIndex]) {
      ...
      if (targetLink && targetLink.offsetParent && targetLink.offsetWidth > 0) {
  ...
  // Don't render the underline until we're hydrated and ready
  const shouldShowUnderline = isHydrated && navReady;
          ...
          <div className="flex items-center h-16 sm:h-18 md:h-20 lg:h-22 xl:h-26 px-4 sm:px-6 lg:px-8 xl:px-0">
            ...
            {!shouldShowMobileMenu && (
              <div className="hidden lg:flex flex-1 justify-end items-center h-full">
                {/* Animated underline element - Only render when ready */}
                {shouldShowUnderline && (
                  <span 
                    ref={underlineRef}
              ...
              <div className="space-y-2  bg-white">
                ...
                <div 
                  ...
                  style={{
                    opacity: 0,
                    transform: 'translateX(20px)',
                    animation: `slideInFromRight 0.3s ease-out forwards ${navItems.length * 0.05}s`
                  }}
                >
                  <div className="flex justify-center items-center space-x-6">
                    <a
                      ...
                      className="p-2 bg-primary-300 text-white hover:bg-primary-400 transition duration-200 rounded-full"
